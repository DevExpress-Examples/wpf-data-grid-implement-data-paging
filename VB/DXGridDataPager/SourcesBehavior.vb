...
Namespace DXGridDataPager
    ...
    Public Class SourcesBehavior
        ...
        Public Shared ReadOnly SourcesProperty As DependencyProperty = DependencyProperty.Register("Sources", GetType(IList), GetType(SourcesBehavior), New PropertyMetadata(Nothing, Sub(d, e) CType(d, SourcesBehavior).OnSourcesChanged(CType(e.NewValue, IList))))
        ...
        Private Sub OnSourcesChanged(ByVal oldSources As IList)
            If DataPager Is Nothing Then Return
            UpdateActualSource(DataPager.PageIndex)
            If Sources IsNot Nothing Then DataPager.ItemCount = Sources.Count
            UnsubsribeSourcesColletion(oldSources)
            SubsribeSourcesColletion(Sources)
            If ActualSource IsNot Nothing Then DataPager.PageIndex = Sources.IndexOf(ActualSource)
            SubscribeDataPager()
        End Sub

        Private Sub UnsubsribeSourcesColletion(ByVal sources As IList)
            If TypeOf sources Is INotifyCollectionChanged Then RemoveHandler CType(sources, INotifyCollectionChanged).CollectionChanged, AddressOf Sources_CollectionChanged
        End Sub

        Private Sub SubsribeSourcesColletion(ByVal sources As IList)
            UnsubsribeSourcesColletion(sources)
            If TypeOf sources Is INotifyCollectionChanged Then AddHandler CType(sources, INotifyCollectionChanged).CollectionChanged, AddressOf Sources_CollectionChanged
        End Sub

        Private Sub UnsubsribeDataPager()
            If DataPager Is Nothing Then Return
            Me.DataPager.PageIndexChanged -= AddressOf DataPager_PageIndexChanged
        End Sub
        ...
        Private Sub Sources_CollectionChanged(ByVal sender As Object, ByVal e As NotifyCollectionChangedEventArgs)
            If Sources IsNot Nothing Then DataPager.ItemCount = Sources.Count
            If ActualSource Is Nothing Then UpdateActualSource(DataPager.PageIndex)
        End Sub
