...
Namespace DXGridDataPager
	Public Class SourcesBehavior
		...
		Public Shared ReadOnly SourcesProperty As DependencyProperty = DependencyProperty.Register("Sources", GetType(IList), GetType(SourcesBehavior), New PropertyMetadata(Nothing, Sub(d, e) (CType(d, SourcesBehavior)).OnSourcesChanged(DirectCast(e.NewValue, IList))))
		Public Property ActualSource() As Object
			Get
				Return DirectCast(GetValue(ActualSourceProperty), Object)
			End Get
		...
		Public Property Sources() As IList
			Get
				Return DirectCast(GetValue(SourcesProperty), IList)
			End Get
		...
		Private Sub UnsubsribeSourcesColletion(ByVal sources As IList)
			If TypeOf sources Is INotifyCollectionChanged Then
				RemoveHandler DirectCast(sources, INotifyCollectionChanged).CollectionChanged, AddressOf Sources_CollectionChanged
			End If
		End Sub
		Private Sub SubsribeSourcesColletion(ByVal sources As IList)
			UnsubsribeSourcesColletion(sources)
			If TypeOf sources Is INotifyCollectionChanged Then
				AddHandler DirectCast(sources, INotifyCollectionChanged).CollectionChanged, AddressOf Sources_CollectionChanged
			End If
		End Sub
